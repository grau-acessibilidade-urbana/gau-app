    ...
    render() {
        return (
            <View style={styles.container}>
                ...
                renderItem={({ item, i }) =>
                    <TouchableOpacity key={i} style={styles.box} activeOpacity={0.7}  onPress={() => this.props.navigation.navigate('HelpDetails')}>
                        <View>
                            <Text style={styles.item}>{item.title}</Text>
                            <Text style={styles.subitem}>{item.subtitle} </Text>
                        </View>
                        <Icon name="chevron-right" size={40} color={commonStyles.colors.secondFontColor} />
                    </TouchableOpacity>
