...
export function getPlacesPredictions(query, currentLocation) {
    return axios.get(`/place/autocomplete/json?input=${query}&types=establishment&components=country:br&key=${Config.GOOGLE_API_KEY}`)
        .then(res => {
...
export function getImageByPhotoReference(photoReference) {
    return axios.get(`/place/photo?maxwidth=400&photoreference=${photoReference}&key=${Config.GOOGLE_API_KEY}`)
        .then(res => res.request.responseURL)
        .catch(err => console.log(err));
}

export function getPlaceDetailsById(currentLocation, id) {
    return axios.get(`/place/details/json?place_id=${id}&key=${Config.GOOGLE_API_KEY}`)
        .then(async (res) => {
...
export function findNearbyPlacesByText(location, text) {
    return axios.get(`/place/nearbysearch/json?location=${location.latitude},${location.longitude}&radius=50000&type=establishment&keyword=${text}&key=${Config.GOOGLE_API_KEY}`)
        .then(res => {
...
export function getDistanceBetweenPlaces(origin, destination) {
    return axios.get(`/distancematrix/json?origins=${origin.latitude},${origin.longitude}&destinations=${destination.latitude},${destination.longitude}&key=${Config.GOOGLE_API_KEY}`)
        .then(res => {
